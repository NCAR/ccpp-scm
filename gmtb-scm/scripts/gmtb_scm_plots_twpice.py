...
plot_dir = "../plots_twpice_gf_t574_test/"

reload(gspr)

nc_fid = Dataset("../output_twpice_gf_test_t574_gf0/output.nc", 'r')
#LES_fid = Dataset("../comparison_data/astex_LES.nc", 'r')
obs_fid = Dataset("../raw_case_input/twp180iopsndgvarana_v2.1_C3.c1.20060117.000000.cdf")
...
forcet = nc_fid.variables['forcet']
forceq = nc_fid.variables['forceq']
...
#read in obs data
day = obs_fid.variables['day'][:]
hour = obs_fid.variables['hour'][:]
...
obs_time = obs_fid.variables['time_offset'][:] #number of seconds since 00Z on 1/17/2006 (starts at 03Z)
#subtract the initial time_offset from all values to get elapsed time since the start of the simulation
obs_time = obs_time - obs_time[start_t_index]
obs_time_hours = obs_time[start_t_index:]/3600.0
obs_levels = obs_fid.variables['lev'][:] #pressure levels in mb

obs_cld = obs_fid.variables['cld'][:]
obs_cld = obs_cld[start_t_index:,:]/100.0
print obs_cld[0,:]
print obs_time_hours
print obs_levels
...
y_ticks_val = [100, 1000, 9]

gspr.contour_plot_firl(obs_time_hours, obs_levels, np.transpose(obs_cld[:,:]), 0.0, 1.0, 'obs. cld. fraction', 'time (h)', 'p (hPa)', plot_dir + 'obs_cld_frc.eps', xticks=x_ticks_val, yticks=y_ticks_val, y_inverted=y_inverted_val, y_log = y_log_val, y_lim = y_lim_val)
...
gspr.plot_profile_multi(phi_axis/9.8, [T_force_tend_mean, dT_dt_lwrad_mean, dT_dt_swrad_mean, dT_dt_PBL_mean, dT_dt_deepconv_mean, dT_dt_shalconv_mean, dT_dt_micro_mean], ['FORCE','LW','SW','PBL','DEEP','SHAL','MICRO'], r'$\frac{\partial T}{\partial t} \left(\frac{K}{day}\right)$', 'Z (m)', plot_dir + 'dT_.eps', y_lim = [0.0, 20000.0])
gspr.plot_profile_multi(phi_axis/9.8, [qv_force_tend_mean, dq_dt_PBL_mean, dq_dt_deepconv_mean, dq_dt_shalconv_mean, dq_dt_micro_mean], ['FORCE','PBL','DEEP','SHAL','MICRO'], r'$\frac{\partial q_v}{\partial t} \left(\frac{g/kg}{day}\right)$ ', 'Z (m)', plot_dir + 'dq_.eps', y_lim = [0.0, 20000.0])
...
#forcing
print time_hours.shape, forcet.shape, T_force_tend.shape
print np.mean(86400*forcet[:,10,:]), np.mean(T_force_tend[:,10,:])
print 86400*forcet[:,10,:], T_force_tend[:,10,:]

forcet_time_series = []
forcet_time_series.append(86400.0*forcet[3:,10,:])
forcet_time_series.append(T_force_tend[3:,10,:])



gspr.plot_time_series_multi(time_hours[3:], forcet_time_series, ['forcet','T-tend'], 'time (h)', 'K/s', plot_dir + '/comp_forcet.eps')
...
for plot in plot_info:
    gspr.contour_plot_firl(time_hours, phi_axis/9.8, np.transpose(plot[0][:,:,0]), np.amin(plot[0][:,0:plot_pres_top_index,0]), np.amax(plot[0][:,0:plot_pres_top_index,0]), plot[1], 'time (h)', 'Z (m)', plot[2], xticks=x_ticks_val, y_lim = [0.0, 20000.0])
