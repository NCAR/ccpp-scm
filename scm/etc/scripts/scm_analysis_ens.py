...
i = scm_datasets[0][:scm_datasets[0].rfind('/')].rfind('/') #find the second-to-last index of '/'
    ...
    for i in range(len(scm_datasets)):
        ...
        for j in range(len(time_slice_labels)):
            ...
            if(profiles_mean['vert_axis'] in locals()):
                ...
                for k in range(len(profiles_mean['vars'])):
                    ...
                    if(profiles_mean['vars'][k] in locals()):
                        ...
                        #spr.plot_profile(vert_axis, mean_data, label, vert_axis_label, ind_dir + '/profiles_mean_' + profiles_mean['vars'][k] + '.eps', y_inverted=y_inverted_val, y_log=y_log_val, y_lim=y_lim_val)
                        if(obs_compare and obs_dict.has_key(profiles_mean['vars'][k])):
                            ...
                            spr.plot_profile_multi_ens(vert_axis, [mean_data], [scm_datasets_labels[i]], label, vert_axis_label_pm, ind_dir + '/profiles_mean_' + profiles_mean['vars'][k] + '.pdf', y_inverted=y_inverted_val_pm, y_log=y_log_val_pm, y_lim=y_lim_val, obs_z=obs_vert_axis, obs_values=obs_mean_data, line_type='color', color_index=i)

                        else:
                            spr.plot_profile_multi_ens(vert_axis, [mean_data], [scm_datasets_labels[i]], label, vert_axis_label_pm, ind_dir + '/profiles_mean_' + profiles_mean['vars'][k] + '.pdf', y_inverted=y_inverted_val_pm, y_log=y_log_val_pm, y_lim=y_lim_val, line_type='color', color_index=i)
                ...
                for multiplot in profiles_mean_multi:
                    ...
                    if all_vars_exist:
                        ...
                        spr.plot_profile_multi_ens(vert_axis, data_list, profiles_mean_multi[multiplot]['vars_labels'], profiles_mean_multi[multiplot]['x_label'], vert_axis_label_pm, ind_dir + '/profiles_mean_multi_' + multiplot + '.pdf', y_inverted=y_inverted_val_pm, y_log=y_log_val_pm, y_lim=y_lim_val, line_type='style', color_index=i)
                    ...
                    if(obs_compare and obs_dict.has_key(time_series['vars'][k])):
                        ...
                        if time_series_resample:
                            ...
                            if(obs_delta_seconds > data_delta_seconds):
                                ...
                                spr.plot_time_series_multi_ens(obs_date_range, [data_time_slice_series_rs_ens], [scm_datasets_labels[i]], 'date', label, ind_dir + '/time_series_' + time_series['vars'][k] + '.pdf', obs_time = obs_date_range, obs_values = obs_data_time_slice, line_type='color', color_index=i)
                                ...
                                spr.plot_time_series_multi_ens(time_h[i][time_slice_indices[j][0]:time_slice_indices[j][1]], [data_time_slice], [scm_datasets_labels[i]], 'time (h)', label, ind_dir + '/time_series_' + time_series['vars'][k] + '.pdf', obs_time = obs_time_time_slice, obs_values = obs_data_time_slice, line_type='color', color_index=i)
                        else:
                            obs_time_time_slice = obs_dict['time_h'][obs_dict['time_slice_indices'][j][0]:obs_dict['time_slice_indices'][j][1]]
                            spr.plot_time_series_multi_ens(time_h[i][time_slice_indices[j][0]:time_slice_indices[j][1]], [data_time_slice], [scm_datasets_labels[i]], 'time (h)', label, ind_dir + '/time_series_' + time_series['vars'][k] + '.pdf', obs_time = obs_time_time_slice, obs_values = obs_data_time_slice, line_type='color', color_index=i)
                    else:
                        spr.plot_time_series_multi(time_h[i][time_slice_indices[j][0]:time_slice_indices[j][1]], [data_time_slice], [scm_datasets_labels[i]], 'time (h)', label, ind_dir + '/time_series_' + time_series['vars'][k] + '.pdf', line_type='color', color_index=i)
                else:
            ...
            for scatter_plot in scatters_mean:
                ...
                spr.plot_scatter_multi([x_mean_data], [y_mean_data], scatters_mean[scatter_plot]['x_label'], scatters_mean[scatter_plot]['y_label'], ind_dir + '/scatter_mean_' + scatter_plot + '.pdf', x_lim = x_lim_val, y_lim = y_lim_val, color_index=i)
    ...
    for j in range(len(time_slice_labels)):
        ...
        if(profiles_mean['vert_axis'] in locals()):
            ...
            for k in range(len(profiles_mean['vars'])):
                ...
                if(profiles_mean['vars'][k] in locals()):
                    ...
                    if(obs_compare and obs_dict.has_key(profiles_mean['vars'][k])):
                        ...
                        spr.plot_profile_multi_ens(vert_axis, mean_data, scm_datasets_labels, label, vert_axis_label_pm, comp_dir + '/profiles_mean_' + profiles_mean['vars'][k] + '.pdf', y_inverted=y_inverted_val_pm, y_log=y_log_val_pm, y_lim=y_lim_val, obs_z=obs_vert_axis, obs_values=obs_mean_data, line_type='color',skill_scores=skill_scores_val)

                    else:
                        spr.plot_profile_multi_ens(vert_axis, mean_data, scm_datasets_labels, label, vert_axis_label_pm, comp_dir + '/profiles_mean_' + profiles_mean['vars'][k] + '.pdf', y_inverted=y_inverted_val_pm, y_log=y_log_val_pm, y_lim=y_lim_val, line_type='color',skill_scores=skill_scores_val)
                else:
                    print 'The variable ' + profiles_mean['vars'][k] + ' found in ' + args.config[0] + ' in the profiles_mean section is invalid.'
                num_plots_completed += 1
                print_progress(num_plots_completed, num_total_plots)
            ...
            for multiplot in profiles_mean_multi:
                ...
                if all_vars_exist:
                    ...
                    spr.plot_profile_multi_ens(vert_axis, data_list_of_list, [profiles_mean_multi[multiplot]['vars_labels'],scm_datasets_labels], profiles_mean_multi[multiplot]['x_label'], vert_axis_label_pm, comp_dir + '/profiles_mean_multi_' + multiplot + '.pdf', y_inverted=y_inverted_val_pm, y_log=y_log_val_pm, y_lim=y_lim_val)
                ...
                if(obs_compare and obs_dict.has_key(time_series['vars'][k])):
                    ...
                    if time_series_resample:
                        ...
                        if(obs_delta_seconds > data_delta_seconds):
                            ...
                            spr.plot_time_series_multi_ens(obs_date_range, data_time_slice_series_rs, scm_datasets_labels, 'date', label, comp_dir + '/time_series_' + time_series['vars'][k] + '.pdf', obs_time = obs_date_range, obs_values = obs_data_time_slice, line_type='color',skill_scores=skill_scores_val)
                        elif(obs_delta_seconds < data_delta_seconds):
                            print 'The case where observations are more frequent than model output has not been implmented yet... '
                        else:
                            obs_time_time_slice = obs_dict['time_h'][obs_dict['time_slice_indices'][j][0]:obs_dict['time_slice_indices'][j][1]]
                            spr.plot_time_series_multi_ens(time_h[0][time_slice_indices[j][0]:time_slice_indices[j][1]], data_time_slice, scm_datasets_labels, 'time (h)', label, comp_dir + '/time_series_' + time_series['vars'][k] + '.pdf', obs_time = obs_time_time_slice, obs_values = obs_data_time_slice, line_type='color',skill_scores=skill_scores_val)
                    else:
                        obs_time_time_slice = obs_dict['time_h'][obs_dict['time_slice_indices'][j][0]:obs_dict['time_slice_indices'][j][1]]
                        spr.plot_time_series_multi_ens(time_h[0][time_slice_indices[j][0]:time_slice_indices[j][1]], data_time_slice, scm_datasets_labels, 'time (h)', label, comp_dir + '/time_series_' + time_series['vars'][k] + '.pdf', obs_time = obs_time_time_slice, obs_values = obs_data_time_slice, line_type='color',skill_scores=skill_scores_val)
                else:
                    spr.plot_time_series_multi_ens(time_h[0][time_slice_indices[j][0]:time_slice_indices[j][1]], data_time_slice, scm_datasets_labels, 'time (h)', label, comp_dir + '/time_series_' + time_series['vars'][k] + '.pdf', line_type='color',skill_scores=skill_scores_val)
            else:
                print 'The variable ' + time_series['vars'][k] + ' found in ' + args.config[0] + ' in the time_series section is invalid.'
            num_plots_completed += 1
            print_progress(num_plots_completed, num_total_plots)
        ...
        for multiplot in time_series_multi:
                ...
                if(obs_compare and obs_dict.has_key(time_series_multi[multiplot]['obs_var'])):
                    ...
                    if time_series_resample:
                        ...
                        if(obs_delta_seconds > data_delta_seconds):
                            ...
                            spr.plot_time_series_multi_ens(obs_date_range, data_list_of_list_rs, [time_series_multi[multiplot]['vars_labels'],scm_datasets_labels], 'date', time_series_multi[multiplot]['y_label'], comp_dir + '/time_series_multi_' + multiplot + '.pdf')#, obs_time = obs_date_range, obs_values = obs_data_time_slice, obs_label = time_series_multi[multiplot]['obs_var_label'])
                        elif(obs_delta_seconds < data_delta_seconds):
                            print 'The case where observations are more frequent than model output has not been implmented yet... '
                        else:
                            obs_time_time_slice = obs_time_h[obs_time_slice_indices[j][0]:obs_time_slice_indices[j][1]]
                            spr.plot_time_series_multi_ens(time_h[i][time_slice_indices[j][0]:time_slice_indices[j][1]], data_list_of_list, [time_series_multi[multiplot]['vars_labels'],scm_datasets_labels], 'time (h)', time_series_multi[multiplot]['y_label'], comp_dir + '/time_series_multi_' + multiplot + '.pdf', obs_time = obs_time_time_slice, obs_values = obs_data_time_slice, obs_label = time_series_multi[multiplot]['obs_var_label'])
                    else:
                        obs_time_time_slice = obs_dict['time_h'][obs_dict['time_slice_indices'][j][0]:obs_dict['time_slice_indices'][j][1]]
                        spr.plot_time_series_multi_ens(time_h[i][time_slice_indices[j][0]:time_slice_indices[j][1]], data_list_of_list, [time_series_multi[multiplot]['vars_labels'],scm_datasets_labels], 'time (h)', time_series_multi[multiplot]['y_label'], comp_dir + '/time_series_multi_' + multiplot + '.pdf', obs_time = obs_time_time_slice, obs_values = obs_data_time_slice, obs_label = time_series_multi[multiplot]['obs_var_label'])
                else:
                    spr.plot_time_series_multi_ens(time_h[i][time_slice_indices[j][0]:time_slice_indices[j][1]], data_list_of_list, [time_series_multi[multiplot]['vars_labels'],scm_datasets_labels], 'time (h)', time_series_multi[multiplot]['y_label'], comp_dir + '/time_series_multi_' + multiplot + '.pdf')
            ...
            for scatter_plot in scatters_mean:
                ...
                spr.plot_scatter_multi(x_mean_data, y_mean_data, scatters_mean[scatter_plot]['x_label'], scatters_mean[scatter_plot]['y_label'], comp_dir + '/scatter_mean_' + scatter_plot + '.pdf', x_lim = x_lim_val, y_lim = y_lim_val)
