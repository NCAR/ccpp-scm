...
#################################################################################################
# Argument list ./create_1D_simdata.py --cases twpice --suites SCM_GFS_v17_p8
#################################################################################################
...
parser.add_argument('-t',   '--time',   help='time to extract, in seconds',           type=int,  default = 3600)
...
def main():
    ...
    for run in run_dict:
        ...
        file_out = "data_CSS_1D."+run["case"] + "." + run["suite"]+".nc"
        ...
        time_diag = ds["time_diag"].values
        try:
            timei = np.argwhere(time_diag==data_time)[0]
        except:
            print('ERROR: can not find requested time in file: ',local_dir+"/output.nc")
            exit()

        # Read in SCM output and aggregate. Store in "var_dict" under "mean"
        for var in var_dict:
            try:
                var["values"]      = ds[var["name"]].values[timei,:,0]
        ...
        for var in var_dict:
            temp = xr.Dataset({var["name"]: (("time","lev"),var["values"])},
                              coords = {"time": np.linspace(0,ntime,ntime),
                                        "lev": np.linspace(0,nlev,nlev)})
