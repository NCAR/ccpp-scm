...
nc_fid = Dataset("../../data/raw_case_input/sgp3hIOPsndgBasedV2.0_ConstrVarAnaX1.c1.19970618.000000.cdf", 'r')

case_period_labels = ['A','B','C','R','S','T','U','X']
#these times omit the 30 minutes in the case specification documentation on purpose since the data in the netCDF file is on the hour (well, 3 seconds past the hour)
case_period_start_datetimes = [datetime.datetime(1997,6,26,hour=23),
                               datetime.datetime(1997,7,7,hour=23),
                               datetime.datetime(1997,7,12,hour=23),
                               datetime.datetime(1997,6,18,hour=23),
                               datetime.datetime(1997,6,22,hour=23),
                               datetime.datetime(1997,6,30,hour=23),
                               datetime.datetime(1997,7,4,hour=23),
                               datetime.datetime(1997,6,18,hour=23)
                              ]
case_period_end_datetimes =   [datetime.datetime(1997,6,30,hour=23),
                               datetime.datetime(1997,7,12,hour=23),
                               datetime.datetime(1997,7,17,hour=23),
                               datetime.datetime(1997,6,22,hour=23),
                               datetime.datetime(1997,6,26,hour=23),
                               datetime.datetime(1997,7,3,hour=23),
                               datetime.datetime(1997,7,8,hour=23),
                               datetime.datetime(1997,7,17,hour=23),
                              ]
...
time_offset = nc_fid.variables['time_offset'][:] - 3 #number of seconds since 00Z on 6/18/1997 (file time starts at 23Z and 03 seconds)
timestamps = base_time + time_offset
...
thetal = np.zeros((levels.size,time_offset.size),dtype=float)
for t in range(time_offset.size):
...
ql = np.zeros((levels.size,time_offset.size),dtype=float)
qi = np.zeros((levels.size,time_offset.size),dtype=float)
tke = np.zeros((levels.size,time_offset.size),dtype=float)
...
w_sub = np.zeros((levels.size,time_offset.size),dtype=float)
...
h_advec_thil = np.zeros((levels.size,time_offset.size),dtype=float)
...
v_advec_thil = np.zeros((levels.size,time_offset.size),dtype=float)
...
h_advec_qt = np.zeros((levels.size,time_offset.size),dtype=float)
...
v_advec_qt = np.zeros((levels.size,time_offset.size),dtype=float)
...
rad_heating = np.zeros((levels.size,time_offset.size),dtype=float)
u_g = np.zeros((levels.size,time_offset.size),dtype=float)
v_g = np.zeros((levels.size,time_offset.size),dtype=float)
...
for i in range(len(case_period_labels)):
    case_period_start_indices.append(data_datetimes.index(case_period_start_datetimes[i]))
    case_period_end_indices.append(data_datetimes.index(case_period_end_datetimes[i]))
