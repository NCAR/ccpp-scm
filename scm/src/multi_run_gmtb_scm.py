...
parser.add_argument('-v', '--verbose',   help='once: set logging level to debug; twice: set logging level to debug and write log to file',action='count')
                ...
                if len(scm_runs.suites) == 1:
                    ...
                    for i, case in enumerate(scm_runs.cases):
                        for j, namelist in enumerate(scm_runs.namelists,1):
                            ...
                            logging.info('Executing process {0} of {1} ({2})'.format(len(scm_runs.namelists)*i+j, len(scm_runs.cases)*len(scm_runs.namelists), command))
                            spawn_subprocess(command, args.timer)
                    ...
                    for i, case in enumerate(scm_runs.cases):
                        for j, suite in enumerate(scm_runs.suites,1):
                            ...
                            logging.info('Executing process {0} of {1} ({2})'.format(len(scm_runs.suites)*i+j, len(scm_runs.cases)*len(scm_runs.suites), command))
                            spawn_subprocess(command, args.timer)
                else:
    ...
    if not args.case and not args.suite and not args.file:
        ...
        for i, case in enumerate(cases):
            for j, suite in enumerate(suites,1):
                command = RUN_SCRIPT + ' -c ' + case + ' -s ' + suite
                logging.info('Executing process {0} of {1} ({2})'.format(len(suites)*i+j, len(cases)*len(suites), command))
                spawn_subprocess(command, args.timer)
