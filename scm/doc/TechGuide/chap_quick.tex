...
\item \execout{-v} specifies the volume mount from ``local'' directory (outside container) to inside the container (bind mount). Allows you to save data. For running the CCPP SCM, the output is being mounted from \execout{/home} inside the container to the \execout{OUT\_DIR} on the ``local" machine.
