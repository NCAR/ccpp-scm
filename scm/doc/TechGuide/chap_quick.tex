...
As an alternative, one can also clone the development branches, although its stability is not guaranteed. To do so,
\begin{enumerate}
    \item Clone the source using
\begin{lstlisting}[language=bash]
...
git checkout master
\end{lstlisting}
\item You may of course also check out development branches of ccpp-physics and ccpp-framework by changing into their respective subdirectories and issuing the same \exec{git checkout} \execsub{master (or other branch)} command.
\end{enumerate}
