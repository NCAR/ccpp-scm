...
\begin{lstlisting}[language=bash]
./ccpp/framework/scripts/ccpp_prebuild.py --config=./ccpp/config/ccpp_prebuild_config.py --static --suites=suite list [--debug]
\end{lstlisting}
where the supplied suite list is a comma-separated list of suite names that exist in the ./ccpp/suites directory.
