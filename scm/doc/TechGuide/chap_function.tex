...
The GMTB SCM uses pressure for the vertical coordinate (lowest index is the surface). There are two choices for generating the vertical coordinate corresponding to a) the 2017 operational GFS based on the Global Spectral Model (GSM) (set \execout{model\_name} $=$ \exec{`GFS'} in the \execout{case\_config} file), and b) the FV3-based GFS (set \execout{model\_name} $=$ \exec{`FV3'} in the \execout{case\_config} file). For both methods, the pressure levels are calculated using the surface pressure and coefficients ($a_k$ and $b_k$). For the GSM-based vertical coordinate, the coefficient data is read from an external file. Only 28, 42, 60, 64, and 91 levels are supported. If using the FV3-based vertical coordinate, it is possible to use potentially any (integer) number of vertical levels. Depending on the vertical levels specified, however, the method of specification of the coefficients may change. Please see the subroutine \execout{get\_FV3\_vgrid} in the source file \execout{gmtb-scm/scm/src/gmtb\_scm\_vgrid.F90} for details. This subroutine was minimally adapted from the source file \execout{fv\_eta.F90} from the v0 release version of the FV3GFS model.
